...
import { LocalServerActions } from "../../app/services/local-server/local-server.slice";

function LocalServerDeleteModal({ id }) {
  ...
  const onDelete = () => {
    dispatch(
      LocalServerActions.delete({
        id,
        onSuccess: () => {
          toast.success("Xoá local server thành công");
  ...
  const contentModal = useMemo(() => {
    return (
      <div>
        <div className="modal-header">Xoá local server</div>
        <div className="modal-body">Bạn có muốn xoá local server này?</div>
  ...
  return (
    <div className="d-flex jc-center">
      <button
        ...
        onClick={() => navigate(`/services/settings/local-server/edit/${id}`)}
      >
        <EditIcon />
      </button>
...
export default LocalServerDeleteModal;
